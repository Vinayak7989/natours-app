...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError("You are not loggend in. Please login to get access", 401)
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  // 3) Update changedPassword At propert for the user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // 3) iF so, update passwrord
  user.password = req.body.password;
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  // User.findByIdAndUpdate will not word.. passConfirm and both pre svae won't run
